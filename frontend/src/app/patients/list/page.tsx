...
export default function PatientsListPage() {
  ...
  const organizeByYearAndMonth = () => {
    ...
    filteredPatients.forEach(patient => {
      if (!patient.dataConsulta) {
        // Pacientes sem data vão para categoria "Sem Data"
        if (!organized['Sem Data']) {
          organized['Sem Data'] = {};
        }
        if (!organized['Sem Data']['00']) {
          organized['Sem Data']['00'] = [];
        }
        organized['Sem Data']['00'].push(patient);
        return;
      }
      ...
      try {
        ...
        if (parts.length !== 3) {
          // Data inválida também vai para "Sem Data"
          if (!organized['Sem Data']) {
            organized['Sem Data'] = {};
          }
          if (!organized['Sem Data']['00']) {
        ...
        // Em caso de erro, adicionar a "Sem Data"
        if (!organized['Sem Data']) {
          organized['Sem Data'] = {};
        }
        if (!organized['Sem Data']['00']) {
  ...
  const getMonthName = (month: string) => {
    if (month === '00') return 'Pacientes sem data de consulta';
    const monthNames = [
                                  ...
                                  {monthPatients.map((patient, index) => (
                                    <tr key={patient._id} className={`transition-colors ${index % 2 === 0 ? '' : 'bg-slate-300'} hover:bg-blue-50`}>
                                      ...
                                      <td className="px-4 py-3 text-center">
                                        {patient.celular && (
                                          <a
                                            href={`https://wa.me/55${patient.celular.replace(/\D/g, '')}`}
                                            target="_blank"
                                            rel="noopener noreferrer"
                                            className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-[#25D366] hover:bg-[#20BA5A] text-white text-xs font-medium rounded transition-colors"
