...
/**
 * Script para corrigir telefones fixos adicionando DD3 na frente
 */
...
async function corrigirTelefones(client) {
    try {
        ...
        const resposta = await new Promise((resolve) => {
            rl.question('Deseja prosseguir com a correção? (s/n): ', (answer) => {
                rl.close();
                resolve(answer);
            });
        });
        ...
        console.log(`Telefones já corretos: ${documentos.length - corrigidos - erros}`);
        if (erros > 0) {
